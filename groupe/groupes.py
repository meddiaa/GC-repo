...
from groupe.creerGroupe import Ui_creerGroupe
import mysql.connector
from groupe.membresGroupe import Ui_Groupe
...
"border-radius:5px;")
        ...
        self.modifier.clicked.connect(self.ouverture_page_membres)
    ...
    def retranslateUi(self, Groupes):
        ...
        item.setText(_translate("Groupes", "Age min"))
        item = self.tableWidget.horizontalHeaderItem(4)
        item.setText(_translate("Groupes", "Age max"))
        item = self.tableWidget.horizontalHeaderItem(5)
        item.setText(_translate("Groupes", "Poids min"))
        item = self.tableWidget.horizontalHeaderItem(6)
        item.setText(_translate("Groupes", "Poids max"))
        Groupes.showMaximized()

        self.filterdropdown.setItemText(0, _translate("Groupes", "choisir"))
        self.filterdropdown.setItemText(1, _translate("Groupes", "ID groupe"))
    ...
    def ouverture_page_membres(self):
        connection, cursor = connect_to_DB()
        Groupe = self.recherche.text()
        cursor.execute("select * from groupes where id_groupe = %s",(Groupe,))
        result = cursor.fetchone()
        if result:
            self.Groupemembres = QtWidgets.QWidget()
            self.ui2 = Ui_Groupe(Groupe)
            self.ui2.setupUi(self.Groupemembres)
            self.Groupemembres.show()
        else :
            self.msg = QtWidgets.QMessageBox()
            self.msg.setIcon(QtWidgets.QMessageBox.Information)
            self.msg.setText("Le groupe que vous voulez afficher n'existe pas")
    ...
    def suppression(self):
            ...
            if critere_id == "ID groupe":
                    query = "select * from groupes where id_groupe = %s"
                    cursor.execute(query, (supp_id,))
                    existence = cursor.fetchone()
                    if existence:
                            query = "DELETE FROM groupes WHERE id_groupe = %s"
                            cursor.execute(query, (supp_id,))
                            ...
                            self.msg.setWindowTitle("Erreur")
                            self.msg3.setText("Attention le groupe saisi n'existe pas ")
                            self.msg3.exec_()
            else:
                    self.msg = QtWidgets.QMessageBox()
                    self.msg.setIcon(QtWidgets.QMessageBox.Information)
    ...
    def affichage(self):
            connection, cursor = connect_to_DB()
            query = "SELECT id_groupe,sexe,sport,age_min,age_max,poids_min,poids_max FROM groupes"
    ...
    def rechercher(self):
        ...
        try:
            try:
                if critere_rech == "ID groupe":
                   query = "SELECT id_groupe,sexe,sport,age_min,age_max,poids_min,poids_max FROM groupes where id_groupe = %s"
                   cursor.execute(query, (texte_rech,))
                elif critere_rech == "Sport":
                   query = "SELECT id_groupe,sexe,sport,age_min,age_max,poids_min,poids_max FROM groupes where sport = %s"
                   cursor.execute(query, (texte_rech,))
                elif critere_rech == "Tranche d'Age":
                        query = "SELECT id_groupe,sexe,sport,age_min,age_max,poids_min,poids_max FROM groupes where %s between age_min and age_max "
                        cursor.execute(query, (texte_rech,))
                elif critere_rech == "Echelle de Poids":
                        query = "SELECT id_groupe,sexe,sport,age_min,age_max,poids_min,poids_max FROM groupes where %s between poids_min and poids_max"
                        cursor.execute(query, (texte_rech,))
